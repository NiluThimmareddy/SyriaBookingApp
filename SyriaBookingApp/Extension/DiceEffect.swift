...
//    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
//        guard let collectionView = collectionView,
//              let attributesArray = super.layoutAttributesForElements(in: rect) else { return nil }
//        
//        let offsetX = collectionView.contentOffset.x
//        let width = collectionView.bounds.width
//        
//        for attributes in attributesArray {
//            let position = (attributes.frame.origin.x - offsetX) / width   // -1, 0, 1, etc.
//            
//            var transform = CATransform3DIdentity
...
//            if position > 0 {
//                // Coming from right
//                attributes.anchorPoint = CGPoint(x: 0, y: 0.5)  // left edge
//                let translateX = -width * 0.5
//                transform = CATransform3DTranslate(transform, translateX, 0, 0)
//                transform = CATransform3DRotate(transform, angle, 0, 1, 0)
//                transform = CATransform3DTranslate(transform, width * 0.5, 0, 0)
//            } else {
//                // Going left
//                attributes.anchorPoint = CGPoint(x: 1, y: 0.5)  // right edge
//                let translateX = width * 0.5
//                transform = CATransform3DTranslate(transform, translateX, 0, 0)
//                transform = CATransform3DRotate(transform, angle, 0, 1, 0)
//                transform = CATransform3DTranslate(transform, -width * 0.5, 0, 0)
