...
/*
 ...
 //                    }
     ...
     @IBAction func sendCodeButtonAction(_ sender: UIButton) {
         ...
         self.getEmailOTP(email: email) { otpResponse in
             self.otpMessageLable.text = "we send verification code to \(email)"
    ...
    func verifyEmailOTPCode(email:String,otp:String,completion: @escaping (VerifyEmailOTPModel?) -> Void) {
     ...
     @IBAction func otpResendButtonAction(_ sender: Any) {
     }
 ...
 extension RegisterMobileNumberVC : UITextFieldDelegate {
     func setUpUI() {
         ...
         setResendButtonTitle(enabled: true)
     ...
     func startResendCountdown() {
         ...
         resendTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
             ...
             if self.totalTime > 0 {
                 self.totalTime -= 1
                 self.setResendButtonTitle(enabled: false)
             } else {
                 self.resendTimer?.invalidate()
                 self.otpResendButton.isEnabled = true
                 self.otpResendButton.alpha = 1.0
                 self.setResendButtonTitle(enabled: true)
             }
         }
     }
     
     func setResendButtonTitle(enabled: Bool) {
         let fullText: String
         if enabled {
             fullText = "Didn’t get it? Resend now"
         } else {
             fullText = "Didn’t get it? Resend — you can resend again in \(totalTime)s"
         }
         
         let attributedString = NSMutableAttributedString(string: fullText)
         let fullRange = (fullText as NSString).range(of: fullText)
         let resendRange = (fullText as NSString).range(of: enabled ? "Resend now" : "Resend")
         
         attributedString.addAttribute(.foregroundColor, value: UIColor.label, range: fullRange)
         attributedString.addAttribute(.font, value: UIFont.systemFont(ofSize: 13, weight: .regular), range: fullRange)
         
         attributedString.addAttribute(.foregroundColor, value: UIColor.systemGreen, range: resendRange)
         attributedString.addAttribute(.font, value: UIFont.systemFont(ofSize: 15, weight: .semibold), range: resendRange)
         
         otpResendButton.setAttributedTitle(attributedString, for: .normal)
     }
