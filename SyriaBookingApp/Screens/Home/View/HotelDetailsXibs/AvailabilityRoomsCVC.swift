...
class AvailabilityRoomsCVC : UICollectionViewCell, UIViewControllerTransitioningDelegate {
    ...
    @IBAction func bookNowButtonAction(_ sender: Any) {
        if let room = selectedRoom {
            let hasNonZeroPrice = room.rates.contains { rate in
                if isLocalRate {
...
extension AvailabilityRoomsCVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let hasNonZeroPrice = selectedRoom.rates.contains { rate in
            if isLocalRate {
                return (rate.localPrice ?? 0) > 0
            } else {
                return rate.price > 0
        ...
        if !hasNonZeroPrice {
            ...
            cell.checkMarkButton.alpha = 0.8
            cell.selectRoomsButton.isUserInteractionEnabled = false
            cell.selectRoomsButton.alpha = 0.8
            cell.roomPriceLabel.alpha = 0.8
    ...
    @objc func checkMarkTapped(_ sender: UIButton) {
        ...
        let isPriceZero = isLocalRate ? (rate.localPrice ?? 0) == 0 : rate.price == 0
        if isPriceZero {
            return
        }
